...
def sub_block(txt, tag, new):
    a = f"<!-- {tag} -->"; b = f"<!-- /{tag} -->"
    return re.sub(f"{re.escape(a)}[\\s\\S]*?{re.escape(b)}",
                  f"{a}\n{new}\n{b}", txt, flags=re.M)
...
def parse_trakt(log):
    ...
    raw = read(log)
    token = "unknown"
    if re.search(r"🧩 Need new refresh token!", raw):
        token = "failed"
    elif re.search(r"Trakt token refresh completed", raw):
        token = "refreshed"
    elif re.search(r"🔐 Trakt token valid", raw):
        token = "valid"
...
def parse_tv(log):
    ...
    for site in list(site_counts.keys()):
        if re.search(rf"FAIL\s+(main|d)\s+{re.escape(site)}", raw):
            site_counts[site]["fail"] = True
    for site,chan,progs in re.findall(r"([a-z0-9\.\-]+).*?-\s*([a-z0-9\-\s]+)\s*-\s*[A-Z][a-z]{{2}}\s+\d{{1,2}},\s*\d{{4}}\s*\((\d+)\s+programs\)", raw, re.I):
        if site in site_counts and int(progs)==0:
            site_counts[site]["warn"].add(re.sub(r"\s+"," ",chan.strip()))
    rows, notes, fails = [], [], []
